...
class RelayPlayer extends Player {
  ...
  readUpstream (packet) {
    ...
    if(this.listenerCount("clientbound") > 0) {
      this.emit('clientbound', des.data, des)
    }
    if(this.listenerCount(`clientbound_${name}`) > 0) {
      this.emit(`clientbound_${name}`, des.data, des)
    }
  ...
  readPacket (packet) {
    ...
    if (this.startRelaying) {
      ...
      if(this.listenerCount("serverbound") > 0) {
        this.emit('serverbound', des.data, des)
      }
      if(this.listenerCount(`serverbound_${des.data.name}`) > 0) {
        this.emit(`serverbound_${des.data.name}`, des.data, des)
      }
